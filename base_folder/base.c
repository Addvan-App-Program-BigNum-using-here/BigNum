...
int main()
{
    ...
    bi_set_from_string(&a, "0x000000000000", 16);
    printf("Printing a:");
    bi_print(a, 16);

    printf("Setting b from string...\n");
    bi_set_from_string(&b, "1234567891232", 10);
    printf("Printing b:");
    bi_print(b, 16);
...
msg bi_delete(bigint **dst) 
{
    if (*dst == NULL) return 0; // 이미 할당 해제되어있음
...
msg bi_print(bigint *dst, int base) 
{
    if (dst == NULL || dst -> a == NULL) return 1;
...
msg bi_assign(bigint **dst, bigint *src) {
    if (*dst == src) return 0; // 자기 자신에게 할당하는 경우
