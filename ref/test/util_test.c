...
{
    FILE *fp = NULL;
    double op_total_time[9] = {
        0,
    };
    double op_exp_time[3] = {
        0,
    };
    byte temp[1] = {0};
    ...
    for (int i = 0; i < test_size; i++)
    {
        ...
        {                                                   // 사전 연산 값이 고정되어 있기에 test_word_size가 기존 사이즈와 같을 때만 수행
    ...
    if (compare_multiplicaiton(16, 120, 16) != COMPARE_MULTIPLICATION_SUCCESS)
        return Test_FAIL; // bigint 곱셈 성능 비교 테스트
    if (compare_squaring(16, 120, 16) != COMPARE_SQUARING_SUCCESS)
        return Test_FAIL; // bigint 곱셈 성능 비교 테스트
    if (compare_division(16, 120, 16) != COMPARE_DIVISION_SUCCESS)
        return Test_FAIL; // bigint 곱셈 성능 비교 테스트
...
EXIT_NEW_DEL:
    ...
    log_msg(result_msg);
    if (Test_file_write(Test_main_file, seperator, APPEND) != FILE_WRITE_SUCCESS)
...
msg test_bi_shift()
{
    ...
    if (Test_file_write(Test_main_file, SHIFT_init, APPEND) != FILE_WRITE_SUCCESS)
        return FILE_WRITE_FAIL;
...
msg test_bi_get_lower()
{
    ...
    if (Test_file_write(Test_main_file, GET_LOWER_init, APPEND) != FILE_WRITE_SUCCESS)
        return FILE_WRITE_FAIL;
...
msg test_bi_cat()
{
    ...
    if (Test_file_write(Test_main_file, CAT_init, APPEND) != FILE_WRITE_SUCCESS)
        return FILE_WRITE_FAIL;
...
msg test_bi_set_from()
{
    ...
    if (Test_file_write(Test_main_file, FROM_init, APPEND) != FILE_WRITE_SUCCESS)
        return FILE_WRITE_FAIL;
    ...
    if (result_msg != Test_BI_SET_FROM_BASE_SUCCESS)
        goto FROM_EXIT;
...
COMAPARE_DIV_EXIT:
    ...
    if (bi_delete(&c) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&d) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    log_msg(result_msg);
    return result_msg;
}
...
msg test_bi_barrett_reduction(OUT double *total_time_barret_reduction, IN bigint **a, IN bigint **barret_N, IN bigint **barret_T, IN char *str)
{
    ...
    if (bigint_to_hex(str, barret_N) == -1)
        goto BARRET_EXIT;
    result_msg = Test_file_write_non_enter(Test_file_barrett_reduction, str, APPEND);
    if (result_msg != FILE_WRITE_SUCCESS)
        goto BARRET_EXIT;
