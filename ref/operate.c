...
msg bi_add(OUT bigint **dst, IN bigint **a, IN bigint **b)
{
    ...
    if (bi_resize(dst, dst_word_len - (carry ^ 1)) != BI_RESIZE_SUCCESS)
    { // carry ^ 1을 한 이유는 마지막 carry가 발생하지 않으면 사이즈를 1 word 줄여도 되서
...
msg bi_sub(OUT bigint **dst, IN bigint **a, IN bigint **b)
{
    ...
    if (bi_resize(b, word_size_b) != BI_RESIZE_SUCCESS)
        return BI_RESIZE_FAIL;
...
msg bi_mul_word(OUT bigint **dst, IN word a, IN word b)
{
    if (*dst == NULL)
    {
        if (bi_new(dst, 2) != BI_ALLOC_SUCCESS)
            return BI_ALLOC_FAIL;
    }
    else if ((*dst)->word_len != 2)
    {
        if (bi_resize(dst, 2) != BI_RESIZE_SUCCESS)
            return BI_RESIZE_FAIL;
    }
...
msg bi_div(OUT bigint **q, OUT bigint **r, IN bigint **a, IN bigint **b, IN int option)
{
    ...
    { // a가 음수, b가 양수인 경우
        ...
        // q와 r 처리
        if (bi_sub(r, b, r) != BI_SUB_SUCCESS)
            return BI_SUB_FAIL; // r = b - r 수행. 여기서 나눗셈 연산 과정에서 b가 변하면 안된다!

        (*q)->sign = 0; // -q
        if (bi_add(q, q, &one) != BI_ADD_SUCCESS)
            return BI_ADD_FAIL; // q = -q - 1 수행 // 일단 여기서 문제 발생
        (*q)->sign = 1;         // q = -q - 1
        result_msg = BI_DIV_SUCCESS;
        goto EXIT_DIV_REFINE;
    }
    ...
    { // a가 b보다 작거나 같은 경우
        ...
        {
            if (bi_resize(q, 1) != BI_RESIZE_SUCCESS)
                return BI_RESIZE_FAIL;
        }
        (*q)->word_len = 1;
        if (!bi_compare)
        { // a == b인 경우
            ...
            if (*r == NULL)
            {
                if (bi_new(r, 1) != BI_ALLOC_SUCCESS)
                    return BI_ALLOC_FAIL;
            }
            else
            {
                if (bi_resize(r, 1) != BI_RESIZE_SUCCESS)
                    return BI_RESIZE_FAIL;
            }
            (*r)->word_len = 1;
            (*r)->a[0] = 0;
        ...
        { // a < b인 경우
            ...
            if (bi_assign(r, a) != BI_SET_ASSIGN_SUCCESS)
                return BI_ALLOC_FAIL; // 어차피 *r == *a 인경우 assign에서 바로 return

            if ((*a)->sign)
            { // a가 음수인 경우
                if (bi_add(r, r, b) != BI_ADD_SUCCESS)
                    return BI_ADD_FAIL;
            }
        }
        result_msg = BI_DIV_SUCCESS;
    ...
    if (bi_new(&temp_q, max_q_size) != BI_ALLOC_SUCCESS)
        return BI_ALLOC_FAIL;
...
EXIT_DIV:
    if (bi_delete(&temp_r) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&temp_q) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
EXIT_DIV_REFINE:
...
msg divc(OUT bigint **q, OUT bigint **r, IN bigint **a, IN bigint **b)
{
    ...
    for (int i = (*a)->word_len - 1; i >= 0; i--)
    {
        ...
        { // bit 단위의 곱셈을 수행하기에
            ...
            { // 여기서 r이 재활용 되지 않은 경우는 상관 없는데 재활용 하는 경우 쓰레기 값이 있는지 확인해볼 필요가 있다. 내부에서 refine 해주기는 한다.
                if (bi_sub(r, r, b) != BI_SUB_SUCCESS)
                    return DIVC_FAIL;
                (*r)->a[(*r)->word_len - 1] = 0;
                (*q)->a[i] |= 1 << j;
            }
...
msg divc_gener(OUT bigint **q, OUT bigint **r, IN bigint **a, IN bigint **b, IN int k)
{
    ...
    for (int i = init_shift_size - 1; i >= 0; i--)
    {
        ...
        if (result_msg != BI_SHIFT_SUCCESS)
            goto EXIT_DIVC_GENER;
        R->a[0] = (*a)->a[i]; // R = R | a[i]
        if (bi_compare_abs(&R, b) >= 0)
        {                                            // 여기가 psuedo code에서 DIVC
...
EXIT_DIVC_GENER:
    if (bi_delete(&R) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&a_p) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&b_p) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&r_p) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    return result_msg;
}
...
EXIT_DIVCC:
    if (bi_delete(&temp_a) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
...
EXIT_SQU:
    if (bi_delete(&C1) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&C2) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&C3) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&mid) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
...
msg bi_squ_karachuba(OUT bigint **dst, IN bigint **a, IN int squ_karachuba_flag)
{
    ...
    // A_1, A_0 계산
    if (bi_resize(&a_1, half_word_len) != BI_RESIZE_SUCCESS)
        return BI_SHIFT_FAIL;
    if (bi_resize(&a_0, half_word_len) != BI_RESIZE_SUCCESS)
        return BI_SHIFT_FAIL;
    ...
    if ((*a)->word_len % 2 == 1)
        a_0->a[loop_len] = (*a)->a[loop_len]; // 길이가 홀수인 경우 반으로 나눌 때
...
EXIT_EXP:
    if (bi_delete(&t[1]) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
EXIT_EXP_T0:
    if (bi_delete(&t[0]) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
EXIT_EXP_T:
    free(t);
    return result_msg;
...
msg bi_exp_R_TO_L(OUT bigint **dst, IN bigint **src, IN bigint **x, IN bigint **n)
{
    ...
    {
        if (bi_resize(dst, 1) != BI_RESIZE_SUCCESS)
            return BI_RESIZE_FAIL;
    }
    (*dst)->a[0] = 1;
...
EXIT_EXP:
    if (flag && bi_delete(dst) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
    if (bi_delete(&t1) != BI_FREE_SUCCESS)
        return BI_FREE_FAIL;
...
msg barret_reduction(OUT IN bigint **dst, IN bigint **a, IN bigint **n, IN bigint **n_barret)
{
    ...
    { // a가 음수인 경우
        ...
        if (result_msg != BI_BARRET_REDUCTION_SUCCESS)
            return result_msg;
