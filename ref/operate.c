...
msg bi_sub(OUT bigint** dst, IN bigint** a, IN bigint** b){
    ...
    for(int i = 0; i < (*a)->word_len; i++){
        temp_a = (*a)->a[i];
        temp_b = (*b)->a[i];
        // borrow2 = ((temp_a < temp_b + barrow) || (temp_b == 0xffffffff && barrow)) ? 1 : 0; // barrow bit 계산
        (*dst)->a[i] = (word)(barrow * 0xFFFFFFFF - temp_b + temp_a);
        //(*dst)->a[i] = (word)(borrow2 * 0xffffffff - temp_b + 1 * borrow2 + temp_a - borrow1);
        barrow = ((temp_a < temp_b + barrow) || (temp_b == 0xffffffff && barrow)) ? 1 : 0; // barrow bit 계산
        // borrow1 = borrow2; // barrow bit 계산
    }
    // 부호 설정
    (*dst)->sign = 0;
