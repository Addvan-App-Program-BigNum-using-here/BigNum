...
msg bi_add(OUT bigint** dst, IN bigint** a, IN bigint** b){
    ...
    if(dst != NULL){
        if(*dst == *a || *dst == *b){ // dst가 a나 b인 경우
            bigint* temp = NULL;
            if(bi_add(&temp, a, b) != BI_ADD_SUCCESS)    return BI_ADD_FAIL;
            if(bi_assign(dst, &temp) != BI_SET_ASSIGN_SUCCESS)    return BI_ADD_FAIL;
            if(bi_delete(&temp) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
            return BI_ADD_SUCCESS;
        }else{ // dst가 a와 b가 아닌 경우
...
msg bi_sub(OUT bigint** dst, IN bigint** a, IN bigint** b){
    ...
    if(dst != NULL){
        if(*dst == *a || *dst == *b){ // dst가 a나 b인 경우
            bigint* temp = NULL;
            if(bi_sub(&temp, a, b) != BI_SUB_SUCCESS)    return BI_SUB_FAIL;
            if(bi_assign(dst, &temp) != BI_SET_ASSIGN_SUCCESS)    return BI_SUB_FAIL;
            if(bi_delete(&temp) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
...
msg bi_mul(OUT bigint **dst, IN bigint **a, IN bigint **b){
    ...
    if(dst != NULL){
        if(*dst == *a || *dst == *b){ // dst가 a나 b인 경우
            bigint* temp = NULL;
            if(bi_mul(&temp, a, b) != BI_MUL_SUCCESS)    return BI_MUL_FAIL;
            if(bi_assign(dst, &temp) != BI_SET_ASSIGN_SUCCESS)    return BI_MUL_FAIL;
            if(bi_delete(&temp) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
...
msg bi_mul_karachuba(OUT bigint **dst, IN bigint **a, IN bigint **b){
    int max_word_len = 0, half_word_len = 0;
//    char str[1024];
    bigint* a_0 = NULL;
    bigint* b_0 = NULL;
    bigint* a_1 = NULL;
    ...
    if(dst != NULL){
        if(*dst == *a || *dst == *b){ // dst가 a나 b인 경우
            bigint* temp = NULL;
            if(bi_mul_karachuba(&temp, a, b) != BI_MUL_SUCCESS)    return BI_MUL_FAIL;
            if(bi_assign(dst, &temp) != BI_SET_ASSIGN_SUCCESS)    return BI_MUL_FAIL;
            if(bi_delete(&temp) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
            return BI_MUL_SUCCESS;
        }else{ // dst가 a와 b가 아닌 경우
    ...
    // 길이의 절반 가져오기
    max_word_len = (*a)->word_len >= (*b)->word_len ? (*a)->word_len : (*b)->word_len;
    ...
    if (bi_refine(*dst) != BI_SET_REFINE_SUCCESS)   goto karachuba_exit;
    return BI_MUL_SUCCESS;

karachuba_exit:
    if(bi_delete(&a_0) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&b_0) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&a_1) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&b_1) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&a_0b_0) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&a_1b_1) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&a_1_a_0) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    if(bi_delete(&b_1_b_0) != BI_FREE_SUCCESS)    return BI_FREE_FAIL;
    return result_msg;
}